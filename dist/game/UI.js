...
let UI = /*#__PURE__*/function () {
  function UI(rows, cols, ratio, loader, input, reset) {
    ...
    _defineProperty(this, "indicator", true);
    ...
    this.loader = loader;
    this.input = input;
    this.drawing = new Drawing(cols * TileSize, rows * TileSize + UISize, ratio);
    this.reset = reset;
  }
  ...
  _createClass(UI, [{
    ...
    value: function draw(score) {
      ...
      this.drawing.draw(({
        ...
        context.fill();
        context.drawImage(foodImage, 0, 0, 48, 48, 22, top + 12, 24, 24);
        ...
        if (success) {
          ...
          context.drawImage(check, 0, 0, 40, 40, x - 28, top + 14, 20, 20);
        }
        ...
        if (this.gameover) {
          ...
          if (isTouchDevice()) {
            context.drawImage(swipe, 0, 0, 202, 168, width / 2 - 50, height - 172, 101, 84);
          } else {
            context.drawImage(keys, 0, 0, 232, 152, width / 2 - 116 / 2, height - 172, 116, 76);
          }
        }
      });
    }
