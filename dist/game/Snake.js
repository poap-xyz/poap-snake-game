...
import { TileSize, Direction, DirectionModifier, DirectionAllowed, LeftEyeModifier, RightEyeModifier } from '../constant';
...
let Snake = /*#__PURE__*/function () {
  function Snake(rows, cols, ratio, loader, input) {
    ...
    _defineProperty(this, "live", false);

    _defineProperty(this, "direction", Direction.RIGHT);

    _defineProperty(this, "grow", false);

    _defineProperty(this, "velocity", 1);

    _defineProperty(this, "nextTimer", 0);

    _defineProperty(this, "target", null);

    _defineProperty(this, "leftEye", null);

    _defineProperty(this, "rightEye", null);

    _defineProperty(this, "segments", []);

    this.loader = loader;
    this.input = input;
    this.drawing = new Drawing(cols * TileSize, rows * TileSize, ratio);
  }
  ...
  _createClass(Snake, [{
    ...
    key: "addSegments",
    value: function addSegments(row, col) {
      for (let i = 0; i < 4; i++) {
    ...
    key: "setTarget",
    value: function setTarget(row, col) {
      this.target = [row, col];
      this.updateEyes();
    }
  }, {
    ...
    value: function move(delta) {
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        segment.y = lerp(segment.y, segment.row * TileSize, acc); //segment.y = segment.row * TileSize
    ...
    key: "renderHead",
    value: function renderHead(context) {
      const {
        x,
        y,
        direction
      } = this.segments[0];
      context.drawImage(this.loader.getImage('snake.head'), frames.head[direction][0] * frameSize, frames.head[direction][1] * frameSize, frameSize, frameSize, x, y, TileSize, TileSize);
      context.drawImage(this.loader.getImage('snake.eye'), 0, 0, 12, 12, this.leftEye.x - 3, this.leftEye.y - 3, 6, 6);
      context.drawImage(this.loader.getImage('snake.eye'), 0, 0, 12, 12, this.rightEye.x - 3, this.rightEye.y - 3, 6, 6);
      /*
